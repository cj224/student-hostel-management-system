...
if (sessionStorage.getItem("login_type") === "houseparent") {
    ...
    contentData = contentData.filter(item => {
        return item.dorm_id == account.dorm_id;
    })
}
localStorage.setItem('curList', "student");
...
function create_tr(item, isOnlyShow) {
    ...
    <tr>
        ...
        <td><span>${item ? item.student_ID : ''}</span></td>
        ...
        <td><span ${isOnlyShow ? '' : 'hidden'}>${item ? item.dorm_number : ''}</span><input ${!isOnlyShow ? '' : 'hidden'} type="text" name="dorm_number" value="${item ? item.dorm_number : ''}"></td>
        <td>
...
function do_save(self, id) {
    if (id) {
        ...
        obj.id = maxId + 1;
        let maxstudent_ID = Math.max(...contentData.map(item => item.student_ID));
        obj.student_ID = maxstudent_ID + 1;
